...
/*
//ac 0ms dfs迭代加深搜索+曼哈顿距离剪枝
//dp[x][y]表示(x,y)点之前走过状态能携带的最多的炸弹数目
//这个迭代加深搜索 其实也可以尝试二分验证,只是最少时间的上界不好确定,
//只能设置为一个很大的值,实际效率可以不如 线性增长最少时间i dfs作验证
...
void dfs(int x,int y,int num,int s,int tot){
    ...
    for(int i=0;i<4;i++){
        ...
        if(mp[nx][ny] == '.'){
            ...
            mp[nx][ny]='.';
            q=dp[nx][ny];
            dfs(nx,ny,num+p,s+1,tot);
            dp[nx][ny]=q;
            mp[nx][ny]=tp;
            if(flag)//要放在mp[nx][ny]迷宫当前位置状态 被改变回来后再一层一层退出
                return;
        }
        else if(mp[nx][ny] == 'X' && num>=1){
            mp[nx][ny]='.';
            q=dp[nx][ny];
            dfs(nx,ny,num-1,s+2,tot);
            dp[nx][ny]=q;
            mp[nx][ny]=tp;
            if(flag)//要放在mp[nx][ny]迷宫当前位置状态 被改变回来后再一层一层退出
                return;
            ...
            flag=1;//在当前规定时间tot内 能刚好到达'D'点,那么从‘S'出发到达'D'点的最少时间就是tot
            return;
        }
    }
}
...
int n,m,dir[4][2]={{-1,0},{1,0},{0,-1},{0,1}},dp1[N][N],dp2[N][N],ans;
char mp[N][N];

void dfs(int x,int y,int num,int s){
    if(dp1[x][y]<=s && dp2[x][y]>=num)
        return;
    if(dp1[x][y]>s)
        dp1[x][y]=s;
    if(dp2[x][y]<num)
