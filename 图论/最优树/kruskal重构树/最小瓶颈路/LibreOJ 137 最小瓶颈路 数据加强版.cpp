...
//可以试试RMQ-st表构建Kruskal重构树 直接O(1)返回最大边权
/*
kruskal重构树相关概念:
...
二叉树并不是满二叉树.可以发现越靠近二叉树中的根节点,对应的点权越大
(对于最小树而言,若是最大树,则靠近根节点的点权越小),点的编号也越大,
对应的两点点最大边权也就越大.实际构造时先构造出儿子节点再依次构造出父亲节点.
而kruskal重构树上任意两叶子节点的lca的点权就是原最小生成树上这两点间路径上的最大边权,
若是最大生成树的话,点权是原最大生成树上这两点间路径上的最小边权.
*/
...
/*
//RMQ-st表 直接o(1)返回最大边权,没有构建kruskal重构树的做法 ac 2681ms
#include <bits/stdc++.h>
